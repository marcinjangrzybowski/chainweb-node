...
tests baseRdb = testGroup "Pact5 SignedList Plugin Tests"
  [ signedListTestTemplate baseRdb
    "signedListValidatorTest"
  ...
  , signedListTestTemplate baseRdb
    "signedListValidatorTestNested"
    simpleExampleMsgProof2
    nestedCapArgs
    "[[\"issue\",\"finp2p\",\"citi:102:d0c3eb56-0fff-4670-adfd-ad291a4314c3\",\"finId\",\"02fd7923740a775c95ce17e9bb7239ff9096689f70db9263a7efb9a9ad08e9fed7\",\"1\"]] \"02fd7923740a775c95ce17e9bb7239ff9096689f70db9263a7efb9a9ad08e9fed7\""
    True Nothing
  , signedListTestTemplate baseRdb "signedListValidatorTestMalformedProof" malformedProof [] "[]" False (Just "Malformed inputs for signature verification")
  , signedListTestTemplate baseRdb "signedListValidatorTestMismatchedCap" simpleExampleMsgProof mismatchedCapArgs "[]" False (Just "Capability arguments do not match proof data")
  ]
...
signedListTestTemplate :: RocksDb -> String -> Aeson.Value -> [PactValue] -> Text -> Bool -> Maybe String -> TestTree
signedListTestTemplate baseRdb testName proof capArgsEvn capArgsContract expectSuccess mErrMsg = testCaseSteps testName $ \step -> runResourceT $ do
  fx <- mkFixture v baseRdb
  liftIO $ do
    deployContract fx step
    step "use plugin"
    prf <- case Aeson.fromJSON proof of
      Aeson.Success p -> pure p
      _ -> assertFailure "Failed to decode proof JSON"

    let cap = CapToken (QualifiedName "K" (ModuleName "m" (Just (NamespaceName "free")))) capArgsEvn
    ...
    poll fx v chain0 [cmdToRequestKey usePlugin]
      >>= P.list [P.match _Just ? P.checkAll
                  [ if expectSuccess then successfulTx
                    else case mErrMsg of
                      Just _ -> P.succeed
                      Nothing -> P.fail "should have trown error..."
                  ]]
     where
     ...
     deployContract fx step = do
       ...
       deploy <- buildTextCmd v
         ...
         $ set cbRPC (mkExec' $ mconcat
             ...
             , "(defun x () (with-capability (K ", capArgsContract ,") 1)))"
             ])
         $ defaultCmd chain0
...
nestedCapArgs = [plist [plist (PString <$> ["issue","finp2p","citi:102:d0c3eb56-0fff-4670-adfd-ad291a4314c3","finId","02fd7923740a775c95ce17e9bb7239ff9096689f70db9263a7efb9a9ad08e9fed7","1"])], PString "02fd7923740a775c95ce17e9bb7239ff9096689f70db9263a7efb9a9ad08e9fed7"]
